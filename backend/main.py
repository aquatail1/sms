...
app = FastAPI(title="SMS Monitoring API", version="0.3.0")
pool = ConnectionPool(conninfo=DATABASE_URL, min_size=1, max_size=10, kwargs={"autocommit": True}, open=False)
    ...
    with pool.connection() as conn, conn.cursor(row_factory=dict_row) as cur:
        cur.execute(
            ...
            SELECT
              ...
              inet_server_addr()::text AS database_host,
              inet_server_port() AS database_port,
              (SELECT COUNT(*) FROM tree_nodes) AS tree_nodes,
              (SELECT COUNT(*) FROM agents) AS agents,
              (SELECT COUNT(*) FROM metrics_resource) AS metrics
            """
        )
        row = cur.fetchone()
    return row
...
def _normalize_node_path(node_path: str) -> list[str]:
    normalized = node_path.strip().strip("/").replace("/", ".")
    labels = [label for label in normalized.split(".") if label]
    if not labels:
        raise HTTPException(status_code=400, detail="node_path is required")
    invalid = [label for label in labels if not LTREE_LABEL_RE.match(label)]
    if invalid:
        raise HTTPException(
            status_code=400,
            detail=f"node_path contains invalid ltree labels: {', '.join(invalid)}",
        )
    return labels


def _ensure_node_path(node_path: str) -> int:
    """Create missing tree nodes for an agent path and return the leaf node id."""
    labels = _normalize_node_path(node_path)
    parent_id = None
    current_path = ""

    with pool.connection() as conn, conn.cursor(row_factory=dict_row) as cur:
        for depth, label in enumerate(labels):
            current_path = label if not current_path else f"{current_path}.{label}"
            cur.execute(
                """
                ...
                SET name = EXCLUDED.name,
                    parent_id = COALESCE(tree_nodes.parent_id, EXCLUDED.parent_id),
                    depth = EXCLUDED.depth,
                    updated_at = NOW()
                RETURNING id
                """,
                (parent_id, label, current_path, depth),
            )
...
def register_agent(payload: RegisterRequest):
    ...
    node_id = _ensure_node_path(payload.node_path)
    agent_id = uuid4()
